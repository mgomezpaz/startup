...
// GetAuth for the currently authenticated user
...
apiRouter.get('/history', verifyAuth, async (req, res) => {
  const user = await findUser('token', req.cookies[authCookieName]);
  const history = await DB.getAnalysisHistory(user.email);
  res.send(history);
});

// Submit a new code analysis
apiRouter.post('/analyze', verifyAuth, async (req, res) => {
  const user = await findUser('token', req.cookies[authCookieName]);
